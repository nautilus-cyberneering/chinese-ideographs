...
def get_modified_base_images(repo):
    ...
    # Git detects only the modification of the dvc file.
    # For example: data/000001/42/000001-42.600.2.tif.dvc
    # but we return the relative path of the modified image: data/000001/42/000001-42.600.2.tif
    # Then, we have to remove the .dvc extentions if it exist.
    all_modified_files_without_dvc_extension = [
        file.removesuffix('.dvc') for file in all_modified_files]

    return filter_base_images(filter_media_library_files(all_modified_files_without_dvc_extension))
...
def add_base_images_in_git(repository, repo_dir, repo_token, repo_base_image_paths, branch_ref):
    ...
    for repo_base_image_path in repo_base_image_paths:
        ...
        print("Base image gitignore path: ", base_image_gitignore_path)
...
def update_base_images_in_git(repository, repo_dir, repo_token, repo_base_image_paths, branch_ref):
    gh = github.Github(repo_token)
    ...
    for repo_base_image_path in repo_base_image_paths:
        ...
        # Files we have to include in the commit:
        # data/000001/42/000001-42.600.2.tif.dvc  (dvc pointer)
        # In this case the .gitignore file should already exists.
        ...
        # Commit message
        base_image_filename = os.path.basename(repo_base_image_path)
        commit_message = f'Update Base image: {base_image_filename}'

        branch = os.path.basename(branch_ref)

        # Debug info
        print("Auto-commit to update Base image: ")
        print("Repo dir: ", repo_dir)
        print("Base image path: ", base_image_path)
        print("Base image pointer path: ", base_image_pointer_path)
        print("Base image gitignore path: ", base_image_gitignore_path)
...
def commit_added_base_images(local_repo, repository, repo_dir, repo_token, branch):
    ...
    commits_for_added_images = add_base_images_in_git(
...
def commit_modified_base_images(local_repo, repository, repo_dir, repo_token, branch):
    ...
    # git commit Base image: dvc pointer, and .gitignore
    commits_for_modified_images = update_base_images_in_git(
...
def auto_commit(repository, repo_dir, repo_token, branch):
    ...
    # TODO: delete Base image if Gold image was deleted.
